...
namespace FormularioUniverso
{
    ...
    {
        private IniciarHilos initForm;

        private List<Calanna.Cecilia._2A.TPFinal.PersonajeSQL> personajesSql;
        ...
        private void btn_filtrar_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Gracias por su consulta");
            }
        }
        ...
        {
            this.lstBuenas.Items.Clear();
            this.lstMalas.Items.Clear();
            this.lstSuper.Items.Clear();

            ///Utilizando la clase ADO, obtener y mostrar todos las peliculas
            ///
            this.personajesSql = Calanna.Cecilia._2A.TPFinal.ADO.ObtenerTodos();
            this.lstTodos.DataSource = this.personajesSql;

            foreach (Calanna.Cecilia._2A.TPFinal.PersonajeSQL item in this.personajesSql)
            {
               
                item.Buena += this.PeliBuena;
                item.Mala += this.PeliMala;
                item.Super += this.PeliSuper;
        ...
        private void ManejadorAgregar(object emisor, EventArgs argumentos)
        {
            ...
            ADO personajeADO = new ADO();
            if (frm.ShowDialog() == DialogResult.OK)
            {
                if (personajeADO.Agregar(frm.PersonajeSql))
                {
                    this.CargarListados();
                }
                else
        ...
        private void ManejadorModificar(object emisor, EventArgs argumentos)
        {
            ...
            Calanna.Cecilia._2A.TPFinal.PersonajeSQL personajesSql = this.personajesSql[i];

            Form1 frm = new Form1(personajesSql);
            frm.StartPosition = FormStartPosition.CenterParent;

            if (frm.ShowDialog() == DialogResult.OK)
            {
                Calanna.Cecilia._2A.TPFinal.ADO ado = new Calanna.Cecilia._2A.TPFinal.ADO(frm.PersonajeSql);

                if (ado.Modificar())
                {
                    this.personajesSql[i] = frm.PersonajeSql;
        ...
        private void ManejadorEliminar(object emisor, EventArgs argumentos)
        {
            ...
            Calanna.Cecilia._2A.TPFinal.PersonajeSQL prod = this.personajesSql[i];

            Form1 frm = new Form1(prod);
            frm.StartPosition = FormStartPosition.CenterParent;
